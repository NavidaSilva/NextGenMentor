...
import Help from '../Pages/settings/mentor/help'; 
...
describe('Help Component', () => {
  ...
  test('sends role field in request body for admin portal tracking', async () => {
    fetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ message: 'Request submitted successfully' }),
    });
    ...
    await waitFor(() => {
      ...
      expect(requestBody).toMatchObject({
        ...
        email: 'john@example.com',
        message: 'Help!',
        role: 'mentor' // This ensures role is sent for admin portal
      });
      expect(requestBody.submittedAt).toBeDefined();
      expect(typeof requestBody.submittedAt).toBe('string');
    });
